...
```{r}
#Find a way to filter in before reading the species in
data <- readOGR('data/') 
```

Get unique names
```{r}
species.names <- unique(data@data$SCINAME)
```
...
```{r}
### Get Data from Chelsea instead

# #Easy to get 2.5 min
# bioclim_data <- getData("worldclim",var="bio",res=2.5)

#But let's get 30 second
bioclim.layer <- dir('data/BIOCLIMdata', "*.tif")
bioclim_data <- stack(paste(getwd(),"/data/BIOCLIMdata/", bioclim.layer, sep=""))
```

##Read in species distribution polygons

Now we want to obtain a sample from our spatial polygon. First we should export our shapefile dataset (from QGIS). Then we read in the shapefile using readOGR (using lapply to make list of dataframes)

```{r, results='hide'}
shps <- dir('data/All_species_R', "*.shp")
shps <- sub('\\.shp$', "", shps)

# #Read in Dale data and restrict
# dale.dat <- read.csv('data/dale_plumage_scores.csv')
# passerine.master.tax <- read.csv('data/Pass_Master_Tax.csv')
# 
# #Is our Dale dataset equivelant to the Jetz master list?
# dale.jetz.anti <- anti_join(passerine.master.tax, dale.dat, by = "binomial")
# #We lose species because they are not included in the Dale dataset (at least not at the time the dataset was created)
# 
# #Restrict shps
# shps.df <- as.data.frame(shps)
# colnames(shps.df) <- "binomial"
# shps.df$iucn <- "yes"
# shps.df$iucn.sciname <- shps.df$binomial
# 
# #Let's see what Passerine bird species are missing from our 
# dale.iucn.match <- left_join(dale.dat[1], shps.df, by = "binomial")
# 
# dale.iucn.match <- dale.iucn.match %>% replace_na(list(iucn = "no"))

# #From this we can start to match missing species in the IUCN
# #Do this manually in excel
# write.csv(dale.iucn.match, "data/dale.iucn.match.csv", row.names = F)

#Read back in the list of taxa where Jetz taxanomic names are matched to IUCN names
taxa.match.df <- read.csv('data/taxa_match.csv')

#Let's double check that our names match with the shapefile names.
anti.taxa.match <- anti_join(taxa.match.df, shps.df, by = 'binomial')
#Ok good, should match. However there might be a few duplicates due to species lumping and classification differences. Better to extract data for these rather than omitting as species ranges should be similar/same
anyDuplicated(taxa.match.df$binomial)

shps.df2 <- data_frame(taxa.match.df$binomial)
shps.jetz <- data_frame(taxa.match.df$jetz.name)
colnames(shps.jetz) <- "binomial"
colnames(shps.df2) <- "binomial"
# #Ok so we lose 1230 species going from Dale data to IUCN (were in Dale but not IUCN)
# #not in dale
# no.dale <- anti_join(shps.df, dale.dat, by = "binomial")
# #not in iucn
# no.iucn <- anti_join(dale.dat, shps.df, by = "binomial")

shps <- as.vector(shps.df2$binomial)
```

We can now use the list of taxa to read in the polygons for each species
```{r}
# shps <- paste(shps, ".shp", sep="")
bird.ranges <- lapply(shps, function(x) {
  assign(x, readOGR(dsn = 'data/All_species_R', layer = x))})
```
